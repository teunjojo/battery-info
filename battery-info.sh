...
energyFull=$(echo | upower -i $(upower -e | grep 'BAT') | grep -oP '(?<=energy-full:         ).*[0-9]')
energy=$(awk -vn="$(echo | upower -i $(upower -e | grep 'BAT') | grep -oP '(?<=energy:              ).*[0-9]')" 'BEGIN{print(n*100)}')
p=$(awk -vn="$energy" -vm="$energyFull" 'BEGIN{printf("%.0f\n",n/m)}')
